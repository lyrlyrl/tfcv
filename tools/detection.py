...
if __name__ == '__main__':
    ...
    if arguments.mode == 'train':
        ...
        if os.path.isdir(checkpoint_dir):
            ...
            for i in historys:
                try:
                    ...
                    print(f'unuseful file or dir {os.path.join(checkpoint_dir, str(i))} remove')
                    continue
                if i > run_id:
                    run_id = i
        run_id += 1
        while True:
            ...
            prefix = 'TF_XLA_FLAGS="--tf_xla_auto_jit=2 --tf_xla_cpu_global_jit" ' if not arguments.noxla else ''
            if len(arguments.gpu_ids) > 1:
                n = len(arguments.gpu_ids)
                prefix = prefix + f'CUDA_VISIBLE_DEVICES={",".join([str(id) for id in arguments.gpu_ids])} horovodrun -np {n} -H localhost:{n} '
                train_command = prefix + train_command
            train_result = subprocess.run(train_command, shell=True)
            print(train_result)
            print(train_result.returncode)
            ...
            eval_result = subprocess.run(eval_command, capture_output=True, shell=True)
            if eval_result.returncode == 0:
                print('eval of epoch finished')
            else:
                pass
            run_id += 1
